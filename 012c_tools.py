...
class RootDetector(keras.Model):
    ...
    def build_model():
        ...
        x            = keras.layers.BatchNormalization()(x)
        x            = keras.layers.Conv2D(1,  (3,3), padding='same', activation='sigmoid', name='bistate')(x)

        x0           = keras.layers.BatchNormalization()(x0)
        x0           = keras.layers.Conv2D(3,  (3,3), padding='same', activation='softmax', name='tristate')(x0)

        m            = keras.Model(in0, dict(bistate=x, tristate=x0))
        return m
    ...
    def predict_patches(self, patches, callback=None):
        ...
        bipatches     = (resultpatches['bistate'][:len(patches)]  + resultpatches['bistate'][len(patches):,:,::-1])/2
        bipatches     = (bipatches > 0.5)*1
        tripatches    = (resultpatches['tristate'][:len(patches)] + resultpatches['tristate'][len(patches):,:,::-1])/2
        tripatches    = np.argmax(tripatches, axis=-1)
        result        = np.where(tripatches>1, tripatches, bipatches[...,0])
        return result
...
def train_model(model, ds_train, ds_valid=None, epochs=15, batch_size=8, progress_callback=lambda e:None):
    ds_train        = ds_train.batch(batch_size)
    ds_valid        = ds_valid.batch(batch_size) if ds_valid is not None else None
    
    callbacks = [
        keras.callbacks.LearningRateScheduler(lambda epoch,lr: lr*0.2 if epoch in [10,13] else lr),
    ...
    model.compile(
        ...
        loss    = {'bistate':weighted_loss(dice_entropy_loss), 'tristate':keras.losses.sparse_categorical_crossentropy}, 
        metrics = {'bistate':metrics, 'tristate':[keras.metrics.SparseCategoricalAccuracy()]},
        loss_weights = {'bistate':1.0, 'tristate':0.1},
    )
...
def load_and_slice(args):
    ...
    #label         = skio.imread(labelfile.decode('utf8'), as_gray=1)>0.5
...
@tf.function
def augment(x,y):
    x,y = (x[:,::-1],y[:,::-1]) if tf.random.uniform((1,))[0]<0.5 else (x,y)
    x,y = (x[::-1],  y[::-1]  ) if tf.random.uniform((1,))[0]<0.5 else (x,y)
    x,y = (tf.transpose(x,(1,0,2)),  tf.transpose(y,(1,0,2)) ) if tf.random.uniform((1,))[0]<0.5 else (x,y)
    return x, y

def reorder(x,y):
    return x, {'bistate':tf.cast(y==1, 'float32'), 'tristate':tf.cast(y, 'float32')[...,0]}
...
def build_dataset(imagefiles, targetfiles, trainmode=False, ensure_caching=False):
    ...
    ds       = tf.data.Dataset.from_tensor_slices( list(zip(imagefiles, targetfiles)) ).map(ds_load_and_slice, -1).unbatch().cache(cachedir.name+'/cache0')
